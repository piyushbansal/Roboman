...
stdscr.addstr(18,55,"welcome to the magenta man",curses.color_pair(1))
stdscr.refresh()
time.sleep(2)
curses.curs_set(0)
curses.noecho()
...
#stdscr=curses.initscr()
	...
	def printing(self):
		...
		i=0
		while i<3:
			stdscr.addch(bulletx[i][0], bulletx[i][1], curses.ACS_UARROW)
			stdscr.addch(bullety[i][0], bullety[i][1], curses.ACS_RARROW)
			i+=1
		time.sleep(0.04)
		i=0
		while i<3:
...
def instructions():	
	...
	stdscr.keypad(1)
	#stdscr.border('#','#','#','#','+','+','+','+')
	stdscr.addstr(5,5,"CONTROLS AND GAMEPLAY:\n\n     1) Use arrow keys to navigate your robot bomb diffuser\n     2) Collect all the defusal kits (D) to earn points and defuse bomb (B) to win\n     3) Do not touch any boundary\n     4) Do not touch any bomb before collecting all defusal kits\n     5) Press any key to start game ,HAve FuN !!\n",curses.color_pair(1))
	stdscr.box()
	stdscr.addstr(20,5,'Loading game...Recommended system configuration -2 GB RAM,1 GB graphics card',curses.color_pair(1))
	stdscr.refresh()
	k=0
	i=0
	j=0
	ls=['|']*100
	while ls[99]!='-' and j<99 and i<99:
		if ls[i]=='|':
			ls[i]='/'
			i+=1
			for items in ls:
				...
				if k>99:
					k=k-99
				time.sleep(0.001)
				stdscr.addch(23,k,items,curses.color_pair(1))
				stdscr.refresh()
		if ls[j]=='/':
			ls[j]='-'
			j+=1
...
def put_bombs_and_defusals():
	...
	i=0
	while i<no_of_defusals:
		stdscr.addch(defusals[i][0],defusals[i][1],'D',curses.color_pair(1))
		stdscr.refresh()
		i+=1
#put_bombs_and_defusals()
...
while level_reached in [1,2]:
	instructions()
	curses.initscr()
	tempscr=curses.newwin(39,150,0,0)
	tempscr.refresh()
	time.sleep(1)
...
print "had fun ? ,new levels coming soon ..."
